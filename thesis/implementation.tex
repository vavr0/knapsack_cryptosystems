...
superincreasing private sequence, a modulus, and a multiplier. In the
implementation, the private sequence is generated by maintaining the current
sum \(S\) of previous private weights. For each new weight, a random increment
\[
...
\[
    w_i = S + \delta_i.
\]
After all private weights are generated, the modulus is chosen as
\[
    m = S + \mu,
\]
where
...
\]
The multiplier is then chosen randomly until it has an inverse modulo \(m\).
These constants are later varied in the parameter-sweep experiments.
...
block size is 128 bits unless another value is given with \texttt{--n}.
