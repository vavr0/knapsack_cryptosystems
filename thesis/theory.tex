...
algorithm~\cite[pp.~216--217, p.~492]{stanoyevitch2011}. The ordinary Euclidean
algorithm first computes $\gcd(r,m)$ by repeated divisions with remainders. In
one division step, a number $A$ is divided by a divisor $D$:
\[
    A = qD + R,
\]
where $q$ is the quotient and $R$ is the remainder. The next step uses the
previous divisor $D$ and the new remainder $R$. This continues until the
remainder becomes $0$. The last non-zero remainder is $\gcd(r,m)$.
...
\[
    W=(2,\,5,\,11,\,23),
\]
with $m=47$ and $r=9$. Here $\gcd(9,47)=1$, so an inverse of $9$ modulo $47$ exists, and $m > 2+5+11+23 = 41$.
...
For decryption, it is not enough to know that this inverse exists; its actual
value is needed. The Euclidean divisions are
\[
    47 = 5\cdot 9 + 2,\qquad 9 = 4\cdot 2 + 1.
\]
The last remainder $1$ confirms that $\gcd(9,47)=1$. The same divisions can be
read backwards to write $1$ as a combination of $9$ and $47$. The second
division gives $1=9-4\cdot2$. From the first division, $2=47-5\cdot9$.
...
\]
so the needed inverse is $r^{-1}\equiv 21 \pmod{47}$. Then
\[
