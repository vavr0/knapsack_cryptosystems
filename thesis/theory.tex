...
algorithm~\cite[pp.~216--217, p.~492]{stanoyevitch2011}. The ordinary Euclidean algorithm first computes $\gcd(r,m)$ by repeated
division with remainders. Since $0<r<m$, the first division uses $m$ as the dividend and $r$ as the divisor.
It gives a quotient $q_1$ and a remainder $R_1$, so
\[
    m = q_1 r + R_1,\qquad 0 \le R_1 < r.
\]
If $R_1$ is not zero, the next division uses $r$ as the dividend and $R_1$ as
the divisor, so
\[
    r = q_2R_1 + R_2,\qquad 0 \le R_2 < R_1.
\]
The algorithm continues by using the divisor and remainder from the previous
step as the next two numbers. It stops when a remainder
becomes $0$, and the last non-zero remainder is $\gcd(r,m)$.

The extended Euclidean algorithm adds backward substitution to these same
divisions. It starts with the equation whose remainder is the greatest common
divisor. If this equation still contains an intermediate remainder, that
remainder is replaced by the expression obtained from the previous division.
Repeating this substitution removes all intermediate remainders and writes the
greatest common divisor using only the original numbers $r$ and $m$. In this
way the algorithm finds integers $a$ and $b$ such that
\[
    ar + bm = \gcd(r,m).
\]
Since key generation chooses $r$ so that $\gcd(r,m)=1$, this gives
\[
    ar + bm = 1.
\]
Reducing this equation modulo $m$ removes the term $bm$, because it is divisible
by $m$, so
\[
    ar \equiv 1 \pmod m.
\]
Thus the coefficient $a$ is the inverse of $r$ modulo $m$, that is,
$r^{-1}\equiv a \pmod m$.
...
\]
Thus the inverse used for decryption is $r^{-1}\equiv 21 \pmod{47}$.
