...
to each letter. Classical ciphers were historically important, yet their
...
section~\cite{diffie1976newDirections}.
...
computational complexity theory provides a way to describe computational
difficulty. The following concepts introduce the basic language used to discuss
this.
...
those decision problems that can be solved in polynomial time with respect to
the input size and they are commonly regarded as efficiently
solvable.~\cite[p.~27]{garey1979computers}
...
$\mathcal{NP}$-complete.~\cite[p.~223]{garey1979computers} Its apparent
hardness was one of the reasons why knapsack-type constructions were originally
considered promising for cryptographic
use.~\cite[pp.~653--654]{diffie1976newDirections}
...
practice. Diffie and Hellman also described a key distribution approach in
which two users communicating over a public channel can obtain a shared secret
key, which can then be used in a conventional symmetric cryptosystem. This
shows that symmetric and asymmetric cryptography do not replace one another,
and can be used together~\cite[pp.~644, 648--649]{diffie1976newDirections}.
...
subset. The related decision problem asks whether such a vector exists. This
decision version is $\mathcal{NP}$-complete.
~\cite[p.~223]{garey1979computers}
...
condition guarantees that this choice is safe. 
