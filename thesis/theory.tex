...
The inverse $r^{-1}$ can be found in polynomial time by the extended Euclidean
algorithm~\cite[pp.~216--217, p.~492]{stanoyevitch2011}. This algorithm is
useful for two reasons. It checks that the chosen multiplier is coprime to the
modulus, and when the gcd is $1$, it also gives the inverse value needed for
decryption. The ordinary Euclidean algorithm first computes $\gcd(r,m)$ by repeated
division with remainders. Since $0<r<m$, the first step writes the result of dividing $m$ by $r$:
\[
    m = q_1 r + \rho_1,\qquad 0 \le \rho_1 < r.
\]
Here $q_1$ is the quotient and $\rho_1$ is the remainder. If $\rho_1\ne 0$,
the next step writes the result of dividing $r$ by this remainder:
\[
    r = q_2\rho_1 + \rho_2,\qquad 0 \le \rho_2 < \rho_1.
\]
Here $\rho_2$ is the new remainder. The algorithm continues in this way until a
zero remainder is reached. The last non-zero remainder is $\gcd(r,m)$.

The extended Euclidean algorithm adds backward substitution to these same
divisions. It starts with the equation where the last non-zero remainder
appears. This remainder is first isolated on the left side. In the notation
above, if this remainder is $\rho_2$, then the second division
\[
    r = q_2\rho_1 + \rho_2
\]
is rewritten as
\[
    \rho_2 = r - q_2\rho_1.
\]
The right side still contains the intermediate remainder $\rho_1$. This
remainder is isolated from the previous division as
\[
    \rho_1 = m - q_1r
\]
and substituted into the current equation. Repeating this step removes all
intermediate remainders. At the end, the greatest common divisor is written
using only the original numbers $r$ and $m$. The concrete substitution with
numbers is shown in the example at the end of this section. In this way the
algorithm finds integers $a$ and $b$ such that
\[
    ar + bm = \gcd(r,m).
\]
When the chosen multiplier passes the gcd check, we have $\gcd(r,m)=1$, so this gives
\[
    ar + bm = 1.
\]
Reducing this equation modulo $m$ removes the term $bm$, so
\[
    ar \equiv 1 \pmod m.
\]
This means that multiplying $r$ by $a$ gives $1$ modulo $m$, which is the
condition for $a$ to be a modular inverse of $r$. Therefore we can write
\[
    r^{-1}\equiv a \pmod m.
\]
...
\[
    W=(2,\,5,\,11,\,23),
\]
with $m=47$ and $r=9$. The modulus is valid because
$m > 2+5+11+23 = 41$. Also, $\gcd(9,47)=1$, so the multiplier has an inverse
modulo $47$. This inverse depends only on the private key values $r$ and $m$,
so it can be computed during key generation.

The inverse is found by the extended Euclidean algorithm. For $m=47$ and
$r=9$, the Euclidean algorithm gives
\[
    47 = 5\cdot 9 + 2,\qquad 9 = 4\cdot 2 + 1.
\]
The last non-zero remainder is $1$, so the numbers are coprime. To get the
inverse, we express this remainder $1$ as a combination of the original numbers
$9$ and $47$. From the
second division,
\[
    1=9-4\cdot2.
...
\]
After reducing modulo $47$, the term $-4\cdot47$ disappears, and therefore
\[
    21\cdot 9 \equiv 1 \pmod{47}.
\]
So the inverse used for decryption is $r^{-1}\equiv 21 \pmod{47}$.
...
\[
    ...
    \end{aligned}
\]
Thus the public weights, which form the public key, are
\[
    B=(18,\,45,\,5,\,19).
\]
...
During decryption, the stored inverse is applied to the ciphertext:
\[
...
\[
    13=11+2,
\]
so the recovered bits are $x=(1,0,1,0)$, which matches the original message.

This is only a toy example chosen to show the arithmetic by hand. Real
cryptographic instances use much larger block lengths. In the experiments, the
implementation is tested with block lengths from $128$ to $4096$ bits.
