...
cryptography. In encryption, the attacker is not interested in if a
...
x = (0,\,1,\,0,\,1).
\]
For general instances, finding such a vector may require checking many possible
subsets.

\subsection*{Superincreasing sequences}

A special easy case is given by superincreasing sequences. A sequence $W$ is
called \emph{superincreasing} if every element is strictly larger than the sum
of all previous elements:
\[
    w_i > \sum_{j=1}^{i-1} w_j \qquad \text{for } i \ge 2.
\]
For such a sequence, subset-sum can be solved greedily. Starting from the
largest weight, the algorithm includes the current weight if it does not exceed
the remaining target value, and otherwise skips it. The superincreasing
condition guarantees that this choice is safe, because all smaller
weights together cannot replace a weight that is too large.

This easy structure is the reason why superincreasing sequences are useful in
the Merkle--Hellman cryptosystem. The private key contains an easy
...
The main parameters are the block length $n$, the private superincreasing
...
it into a public sequence by modular transformation with intention to look
hard. First, a superincreasing sequence $W=(w_1,\dots,w_n)$ is chosen as the
private sequence. Then a modulus $m$ larger than $\sum_{i=1}^n w_i$ and a
multiplier $r$ satisfying $\gcd(r,m)=1$ are selected. The public sequence
$B=(b_1,\dots,b_n)$ is computed from the private sequence using the multiplier
$r$ modulo $m$, where
\[
...
The reason decryption works is the relation between the public and private
weights. Since $b_i \equiv r w_i \pmod m$, multiplying the ciphertext by
$r^{-1}$ gives
\[
    r^{-1}C \equiv \sum_{i=1}^{n} x_i w_i \pmod m.
\]
