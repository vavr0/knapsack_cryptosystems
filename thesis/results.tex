...
A separate experiment was also run for the iterated variant. In this
experiment, the number of total modular transformation layers was changed to
\[
    L \in \{2,3,4,5\}.
\]
...
In demo and benchmark modes, decryption was run independently after encryption
on the produced ciphertext. This means that the decryption function used the
ciphertext and the key material, not the original unencrypted input. The
recovered bit sequence was then compared with the original input, so this
served as a correctness check for each run. If the comparison failed, the
program returned an error value. For plaintext messages, the same check was done
after converting the message into bit blocks and reconstructing the decrypted
output. In solver mode, the bit vector recovered by the brute-force or
meet-in-the-middle solver was also compared with the generated message.
...
and decryption for one fixed-size block. For the smallest block sizes, the
measured times are tiny. Therefore, the exact order of the variants in this
part of the graph should not be overinterpreted. The growth with increasing
\(n\) is more visible.
...
private sequence and the size of the resulting key material. The reason is that
the modulus $m$ is chosen above the private sum $S$, and the upper bound of each
public weight is higher as a consequence. In the experiment, increasing the
increment bound from \(1\) to \(128\) raised the average private-sum bit length
from \(128\) to \(135\) bits for \(n=128\), and from \(4096\) to \(4103\) bits
for \(n=4096\). 

A similar experiment was run for \texttt{MH\_DEFAULT\_MARGIN\_FACTOR}. In the
generator, this constant is multiplied by the block size \(n\), so the margin
\(\mu\) is sampled from \(1\) to \(\texttt{MH\_DEFAULT\_MARGIN\_FACTOR}\cdot
n\). In contrast with \texttt{MH\_DEFAULT\_DELTA\_MAX}, changing this value had
little to no visible effect on the reported density. The added margin was small
compared with the private sum, so the bit length of the generated public
weights stayed almost unchanged.
...
vertical axis is logarithmic, so the almost straight orange curve still
represents exponential growth. The meet-in-the-middle curve is also smoother
because, for a fixed \(n\), the implementation builds and sorts full tables of
partial sums before the final search. Brute force stops as soon as it finds a
matching bit vector, so its time depends more on where this vector appears in
the search order. The brute-force solver becomes expensive already for small
values of \(n\). In these measurements, it was tested only up to \(n=22\),
where the average running time was about \(188\) ms. The meet-in-the-middle
solver handled larger values in this experiment. It reached \(n=40\), where the
average running time was about \(757\) ms.
